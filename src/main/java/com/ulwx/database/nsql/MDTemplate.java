...
import com.ulwx.tool.support.FileUtils;
...
import com.ulwx.tool.support.type.TBoolean;
...
public final class MDTemplate {
	...
	public static void startClass(StringBuilder sb, String className, String packageName, TInteger tabNum) {
		...
		sb.append("import com.ulwx.database.nsql.NFunction;\n");
		sb.append("import static com.ulwx.database.nsql.NFunction.*;\n");
		...
		handLine = StringUtils.replaceAll(handLine, "\\$\\{(" + javaExpressReg + ")}", 1, new GroupHandler() {
			...
			public String handler(String groupStr) {
				...
				if (str.startsWith("=")) {
					...
					//
					return "\"+(" + "com.ulwx.database.nsql.MDTemplate.getResultString(\"" + strs[0].trim() + "\", \""
							+ strs[1].trim() + "\", args)" + ")+\"";
				} else {
					return "\"+(" + "args.get(\"" + groupStr + "\") " + ")+\"";
				}
	 ...
	 * 
	 * @param packageName
	 *            指定包名 例如指定 com.ulwxbase.dao 则会去找com/ulwxbase/dao目录查找
	 * @param className
	...
	public static void main(String[] args) throws Exception {
		...
		// String
		// s=parseFromFileToJava("com.ulwx.database.test.SysRightDao",mp);
		// String
