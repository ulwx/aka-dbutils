...
import com.ulwx.database.*;
import com.ulwx.database.cacherowset.com.sun.rowset.CachedRowSetImpl;
import com.ulwx.database.utils.DbConst;
import com.ulwx.database.utils.Table2JavaNameUtils;
import com.ulwx.tool.support.*;
import com.ulwx.tool.support.type.TResult2;
import com.ulwx.tool.support.type.TString;
...
public class SqlUtils {
	...
	public static Map<String,String> dbEscapeLefChar=new HashMap<String,String>();
	static{
		dbEscapeLefChar.put(DataBase.DataBaseType.MYSQL, "`");
		dbEscapeLefChar.put(DataBase.DataBaseType.MS_SQL_SERVER, "[");
		dbEscapeLefChar.put(DataBase.DataBaseType.MS_SQL_SERVER_2005, "[");
		dbEscapeLefChar.put(DataBase.DataBaseType.ORACLE, "\"");
		dbEscapeLefChar.put(DataBase.DataBaseType.DB2, "");
		dbEscapeLefChar.put(DataBase.DataBaseType.H2, "");
		dbEscapeLefChar.put(DataBase.DataBaseType.HSQL, "");
		dbEscapeLefChar.put(DataBase.DataBaseType.POSTGRE, "");
		dbEscapeLefChar.put(DataBase.DataBaseType.SYBASE, "");
		dbEscapeLefChar.put(DataBase.DataBaseType.SQLITE, "");
		dbEscapeLefChar.put(DataBase.DataBaseType.OTHER, "");
		dbEscapeLefChar.put("", "");
		dbEscapeLefChar.put(null, "");
	}
	
	public static Map<String,String> dbEscapeRightChar=new HashMap<String,String>();
	
	static{
		dbEscapeRightChar.put(DataBase.DataBaseType.MYSQL, "`");
		dbEscapeRightChar.put(DataBase.DataBaseType.MS_SQL_SERVER, "]");
		dbEscapeRightChar.put(DataBase.DataBaseType.MS_SQL_SERVER_2005, "]");
		dbEscapeRightChar.put(DataBase.DataBaseType.ORACLE, "\"");
		dbEscapeRightChar.put(DataBase.DataBaseType.DB2, "");
		dbEscapeRightChar.put(DataBase.DataBaseType.H2, "");
		dbEscapeRightChar.put(DataBase.DataBaseType.HSQL, "");
		dbEscapeRightChar.put(DataBase.DataBaseType.POSTGRE, "");
		dbEscapeRightChar.put(DataBase.DataBaseType.SYBASE, "");
		dbEscapeRightChar.put(DataBase.DataBaseType.SQLITE, "");
		dbEscapeRightChar.put(DataBase.DataBaseType.OTHER, "");
		dbEscapeRightChar.put("", "");
		dbEscapeRightChar.put(null, "");
	}
	...
	private static String exportJavaBean(String dbpool,String tableName, String tableRemark,
		...
		sb.append("import com.ulwx.tool.support.ObjectUtils;\n");
		sb.append("\n/*********************************************\n");
		...
		sb.append("public class " + className + " extends com.ulwx.database.MdbOptions implements java.io.Serializable {\n\n");
		...
		while (i.hasNext()) {
			...
			// System.out.println("sssss "+co.getType_name());
	...
	public static void exportTables(String pool, String schema,
		...
		try {
			...
			for (int i = 0; i < tablelist.size(); i++) {
				...
				//System.out.println(ObjectUtils.toString(columMap));
...
//		 System.out.println(SqlUtils.generateNotParmatersSql(sql, list));
	...
	public static String generateSelectSqlBySelectObject(String dbpoolName,Object selectObject,Class reflectClass,
			Map<Integer, Object> returnvParameters, Map<String, String> exmap,String dataBaseType)
			throws Exception {
		...
		try {
			...
			while (inames.hasNext()) {
				...
				if (SqlUtils.checkedSimpleType(t)) {// 简单类型
					...
					if (value != null) {
						where = where + dbEscapeLefChar.get(dataBaseType)+getColumName(dbpoolName,name)+dbEscapeRightChar.get(dataBaseType) + "=? and ";
	...
	public static String generateSelectSql(String dbpoolName,Object selectObject,Class reflectClass,
		...
		try {
			...
			for (int n = 0; n < keys.length; n++) {
				...
				where = where + dbEscapeLefChar.get(dataBaseType)+getColumName(dbpoolName,keys[n])+dbEscapeRightChar.get(dataBaseType) + "=?";
				if (n < keys.length - 1) {
					where = where + " and ";
				}
				returnvParameters.put(index++, value);
		...
		try {
			...
			while (i.hasNext()) {
				...
				if (SqlUtils.checkedSimpleType(t)) {// 简单类型
					...
					if (colValue != null) {
						colPart = colPart + dbEscapeLefChar.get(dataBaseType)+getColumName(dbpoolName,name)+dbEscapeRightChar.get(dataBaseType) + "=?,";

						returnvParameters.put(index++, colValue);
					}else{
						if(!ignoreNull){
						   colPart = colPart  +dbEscapeLefChar.get(dataBaseType)+getColumName(dbpoolName,name)+dbEscapeRightChar.get(dataBaseType) + "=null,";
						}
					}
	...
	public static String generateInsertSql(String dbpoolName,String[] properties,
			...
			Map<String, String> exmap, boolean ignoreNull,String dataBaseType) throws Exception {
		String sql = "";
		
		String className = reflectClass.getSimpleName();

		sql = "insert into " +dbEscapeLefChar.get(dataBaseType)+ getTableName(dbpoolName,className) +dbEscapeRightChar.get(dataBaseType);
		String colPart = "(";
	...
	public static String getSqlWherePartAndSetParms(String dbpoolName,String sql,
			String property, Object[] objs, Map<Integer, Object> vars) {

		Integer n = (Integer) CollectionUtils.getMaxKeyInMap(vars);
		sql = sql
				+ "and "
				+ getColumName(dbpoolName,property)
				+ " in "
				+ "("
				+ StringUtils.arrayToCommaDelimitedString(ArrayUtils.nCopy("?",
						objs.length)) + ")";
