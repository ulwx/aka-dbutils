...
import com.ulwx.database.page.interceptor.SQLHelp;
import com.ulwx.database.page.model.DBMS;
import com.ulwx.database.sql.BeanUtils;
 ...
 * DataBaseSet rs = db.doCachedPageQuery(sqlx, vparms, page, perPage, pageUtils, null);
 * 
 * while (rs.next()) {
 ...
 * </p>
 */
public class DataBaseImpl implements NoSqlOperation, AutoCloseable, DataBase {
	...
	public DataBaseSet doCachedPageQuery(String sqlQuery, Map<Integer, Object> vParameters, int page, int perPage,
		...
		return new DataBaseSet(
				this.doCachedPageQuery(sqlQuery, args, page, perPage, pageUtils, this.getAutoCommit(), countSql));
	}

	@Override
	public DataBaseSet query(String sqlQuery, Map<Integer, Object> vParameters, int page, int perPage,
			PageBean pageUtils, String countSql) throws DbException {
		return this.doCachedPageQuery(sqlQuery, vParameters, page, perPage, pageUtils, countSql);
	}
	...
	public <T> List<T> doPageQueryObject(String sqlQuery, Map<Integer, Object> args, int page, int perPage,
		...
		String pageSql = this.getPagedSql(sqlQuery, vParameters, page, perPage, pageUtils, countSql, rsStart, rsEnd);
	...
	public <T> List<T> query(String sqlQuery, Map<Integer, Object> args, int page, int perPage, PageBean pageUtils,
		...
		return this.doPageQueryObject(sqlQuery, args, page, perPage, pageUtils, rowMapper, countSql);
	}
	...
	public List<Map<String, Object>> doPageQueryMap(String sqlQuery, Map<Integer, Object> args, int page, int perPage,
		...
		List<Map<String, Object>> list = doPageQueryObject(sqlQuery, args, page, perPage, pageUtils,
	...
	public List<Map<String, Object>> queryMap(String sqlQuery, Map<Integer, Object> args, int page, int perPage,
			PageBean pageUtils, String countSql) throws DbException {
		return this.doPageQueryMap(sqlQuery, args, page, perPage, pageUtils, countSql);
	}
	...
	private CachedRowSet doCachedPageQuery(String sqlQuery, Collection<Object> vParameters, int page, int perPage,
			PageBean pageUtils, boolean close, String countSql) throws DbException {
		...
		try {
			...
			TInteger rsStart = new TInteger();
			TInteger rsEnd = new TInteger();
			String pageSql = this.getPagedSql(sqlQuery, vParameters, page, perPage, pageUtils, countSql, rsStart,
					rsEnd);
			if (StringUtils.isEmpty(pageSql)) {
	...
	public <T> List<T> query(Class<T> clazz, String sqlQuery, Map<Integer, Object> vParameters, int page, int perPage,
			PageBean pageUtils, String countSql) throws DbException {
		return this.doPageQueryClass(clazz, sqlQuery, vParameters, page, perPage, pageUtils, countSql);
	}
	...
	private <T> List<T> doPageQueryClass(Class<T> clazz, String sqlPrefix, String sqlQuery,
		...
		TInteger rsStart = new TInteger();
		TInteger rsEnd = new TInteger();
		String pageSql = this.getPagedSql(sqlQuery, args, page, perPage, pageUtils, countSql, rsStart, rsEnd);
		if (StringUtils.isEmpty(pageSql)) {
			return new ArrayList<T>();
		}
	...
	private String getPagedSql(String sqlQuery, Collection<Object> vParameters, int page, int perPage,
			PageBean pageUtils, String countSql, TInteger rsStart, TInteger rsEnd) throws DbException {
		...
		try {
			...
			if (pageUtils.isEmpty()) {
				return "";
			}
			int begin = 0;
			int end = 0;
			if (this.dataBaseType == DataBaseType.MYSQL) {
				sql = sqlQuery.trim() + " limit " + pageUtils.getStart() + "," + pageUtils.getPerPage();
				begin = 0;
				...
				sql = "select top " + pageUtils.getEnd() + " t.* from (" + sqlQuery + ") t";
				...
				sql = SQLHelp.generatePageSql(sqlQuery, pageUtils, DBMS.SQL_SERVER_2005);

				begin = 0;
				end = pageUtils.getEnd();

			} else if (this.dataBaseType == DataBaseType.DB2) {
				sql = SQLHelp.generatePageSql(sqlQuery, pageUtils, DBMS.DB2);
				begin = 0;
				end = pageUtils.getEnd();
			} else if (this.dataBaseType == DataBaseType.H2) {
				sql = SQLHelp.generatePageSql(sqlQuery, pageUtils, DBMS.H2);
				begin = 0;
				end = pageUtils.getEnd();
			} else if (this.dataBaseType == DataBaseType.HSQL) {
				sql = SQLHelp.generatePageSql(sqlQuery, pageUtils, DBMS.HSQL);
				begin = 0;
				end = pageUtils.getEnd();
			} else if (this.dataBaseType == DataBaseType.POSTGRE) {
				sql = SQLHelp.generatePageSql(sqlQuery, pageUtils, DBMS.POSTGRE);
				begin = 0;
				end = pageUtils.getEnd();
			} else if (this.dataBaseType == DataBaseType.SQLITE) {
				sql = SQLHelp.generatePageSql(sqlQuery, pageUtils, DBMS.SQLITE);
				begin = 0;
				end = pageUtils.getEnd();
			} else if (this.dataBaseType == DataBaseType.SYBASE) {
				sql = SQLHelp.generatePageSql(sqlQuery, pageUtils, DBMS.SYBASE);
				begin = 0;
				end = pageUtils.getEnd();
				...
				sql = "select xx.* from ( select t.*,row_number() over" + "(" + orderStr.getValue()
						+ ")as rownumer from (" + sqlQuery + ")t )xx where rownumer>" + pageUtils.getStart()
						+ " and rownumer<=" + pageUtils.getEnd();
				begin = 0;
				end = pageUtils.getEnd();
			} else {
	...
	public static void main(String[] args) throws Exception {
		...
		System.out.println(getLastTwoClassName("eeeeee.aa.eeee.a.b.d.c.d.d.d.d"));
	...
	// public <T> int del( T deleteObject)
	// throws Exception {
	// return this.excuteDeleteClass( deleteObject);
	// }
	@Override
