 ...
 * 
 * Connection con=getConnection(); String
 * query="select * from my_table where name=? or address=?"; PreparedStatement
 * p=con.prepareStatement(query); p.setString(1, "bob"); p.setString(2,
 * "123 terrace ct"); ResultSet rs=p.executeQuery();
 * 
 * can be replaced with:
 * 
 * Connection con=getConnection(); String
 * query="select * from my_table where name=:name or address=:address";
 * NamedParameterStatement p=new NamedParameterStatement(con, query);
 * p.setString("name", "bob"); p.setString("address", "123 terrace ct");
