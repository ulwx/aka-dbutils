...
public final class NSQL {
	...
	public static NSQL parseScript(String sql, final Map<String, Object> args, final boolean isStoredProc, NSQL nsql) {
		...
		String exeSql = StringUtils.replaceAll(sql,
				"#\\{(\\s*(\\[Array\\])?\\%?\\s*" + $javaNameReg + "\\s*\\%?\\s*)\\}", 1, new GroupHandler() {
					...
					public String handler(String groupStr) {
						...
						//防止注入式攻击
						ESAPI.encoder().encodeForSQL(new MySQLCodec(MySQLCodec.Mode.STANDARD),retValue);
						return retValue;
					}
